...
void AncillaryDataTable::UpdateFrameData(AncillaryDataStruct* newAncData, HDRMetadataStruct* hdrMetadata)
{
	...
	// Static HDR Metadata
	m_hdrMetadataValues.replace(0, hdrMetadata->electroOpticalTransferFunction);
	m_hdrMetadataValues.replace(1, hdrMetadata->displayPrimariesRedX);
	m_hdrMetadataValues.replace(2, hdrMetadata->displayPrimariesRedY);
	m_hdrMetadataValues.replace(3, hdrMetadata->displayPrimariesGreenX);
	m_hdrMetadataValues.replace(4, hdrMetadata->displayPrimariesGreenY);
	m_hdrMetadataValues.replace(5, hdrMetadata->displayPrimariesBlueX);
	m_hdrMetadataValues.replace(6, hdrMetadata->displayPrimariesBlueY);
	m_hdrMetadataValues.replace(7, hdrMetadata->whitePointX);
	m_hdrMetadataValues.replace(8, hdrMetadata->whitePointY);
	m_hdrMetadataValues.replace(9, hdrMetadata->maxDisplayMasteringLuminance);
	m_hdrMetadataValues.replace(10, hdrMetadata->minDisplayMasteringLuminance);
	m_hdrMetadataValues.replace(11, hdrMetadata->maximumContentLightLevel);
	m_hdrMetadataValues.replace(12, hdrMetadata->maximumFrameAverageLightLevel);
	m_hdrMetadataValues.replace(13, hdrMetadata->colorspace);
...
QVariant AncillaryDataTable::data(const QModelIndex& index, int role) const
{
	...
	if ((index.row() >= (kAncillaryDataTypes.size() + kHDRMetadataTypes.size())) || (index.column() >= kAncillaryTableColumnCount))
		return QVariant();
	...
	if (role == Qt::DisplayRole)
	{
		...
		else if (index.column() == static_cast<int>(AncillaryHeader::Values))
		{
			if (index.row() < kAncillaryDataTypes.size())
				...
				return m_hdrMetadataValues.at(index.row() - kAncillaryDataTypes.size());
		}
	}
