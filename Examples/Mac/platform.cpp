...
void StringToCharArray(STRINGOBJ bmdStr, char* charArray, unsigned int arrayLength)
{
	...
	CFStringGetBytes(bmdStr, range, kCFStringEncodingUTF8, '?', false, (UInt8*)charArray, arrayLength-1, &bytesUsed);
...
void StringToStdString(STRINGOBJ bmdStr, std::string& stdStr)
{
	...
	try
	{
		CFIndex len = 1 + CFStringGetMaximumSizeForEncoding(CFStringGetLength(bmdStr), kCFStringEncodingUTF8);
