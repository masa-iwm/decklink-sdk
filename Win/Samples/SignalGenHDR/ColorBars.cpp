...
void FillBT2111ColorBars(CComPtr<IDeckLinkMutableVideoFrame>& colorBarsFrame, EOTFColorRange range)
{
	uint32_t*	nextWord;
	uint32_t	width;
	uint32_t	height;
...
void FillLineBars(ColorBarsPattern& pattern, EOTFColorRange colorRange, std::vector<Color12BitRGB>& lineBuffer)
{
	for (auto& iter : pattern)
	{
		...
		uint32_t barWidth = iter.second * ((uint32_t)lineBuffer.capacity() / kHD1080Width);
...
void FillLineRamp(ColorBarsPattern& pattern, EOTFColorRange colorRange, std::vector<Color12BitRGB>& lineBuffer)
{
	...
	for (auto& iter : pattern)
	{
		...
		{
			// Column widths are based on HD, scale for 4K/8K
			uint32_t endColumn = (iter.second + 1) * ((uint32_t)lineBuffer.capacity() / kHD1080Width) - 1;
